...
export const realtimeproductsView = async (req = request, res = response) => {
  try {
    const user = req.session.user;
    console.log(
      "Renderizando vista de productos en tiempo real con usuario:",
      user
    );
    ...
    console.error(
      "Error al renderizar la vista de productos en tiempo real:",
      error
    );
    return res.status(500).send("Error interno del servidor");
  }
  ...
  if (req.file) {
    ...
    if (!isValidExtension)
      return res.status(400).json({
        msg: "La extensión del archivo no es válida, utilice formato de imagen",
      });
...
export const registerPost = async (req = request, res = response) => {
  try {
    if (!req.user) {
      console.log(
        "Error en el registro, redirigiendo a la página de registro."
      );
